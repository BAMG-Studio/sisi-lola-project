...
st.set_page_config(page_title="Sisi Lola Training Metrics", page_icon="🤖", layout="wide", initial_sidebar_state="expanded")

st.markdown("""
<style>
    .main-header { font-size: 2.5rem; font-weight: bold; color: #FF6B35; text-align: center; padding: 1rem; }
...
def get_ingestion_stats(days: int = 30) -> Dict[str, Any]:
    ...
    df_items = pd.read_sql_query(f"SELECT * FROM ingested_items WHERE ingested_at >= '{cutoff}'", conn)
    df_runs = pd.read_sql_query(f"SELECT * FROM ingestion_runs WHERE started_at >= '{cutoff}' ORDER BY started_at DESC", conn)
    conn.close()
    return {"items": df_items, "runs": df_runs, "total_items": len(df_items), "total_runs": len(df_runs)}
...
def get_generation_metrics(days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        cutoff = (datetime.now() - timedelta(days=days)).isoformat()
        df = pd.read_sql_query(f"SELECT * FROM generation_metrics WHERE end_time >= '{cutoff}'", conn)
        conn.close()
        return {"data": df, "total": len(df), "total_cost": df["cost_usd"].sum() if "cost_usd" in df.columns else 0}
...
def get_training_runs(days: int = 90) -> pd.DataFrame:
    ...
    try:
        conn = sqlite3.connect(PIPELINE_DB)
        cutoff = (datetime.now() - timedelta(days=days)).isoformat()
        df = pd.read_sql_query(f"SELECT * FROM training_runs WHERE start_time >= '{cutoff}' ORDER BY start_time DESC", conn)
        conn.close()
        return df
    except:
        return pd.DataFrame()
...
with st.sidebar:
    ...
    st.divider()
    time_range = st.selectbox("Time Range", options=[7, 14, 30, 60, 90], index=2, format_func=lambda x: f"Last {x} days")
    st.divider()
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 Overview", "📥 Data Ingestion", "🎯 Training Runs", "💰 Cost Tracking", "🔧 Quality Metrics"])

with tab1:
    st.header("System Overview")
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric(label="🎤 Voice Samples", value=ingestion_stats.get("items", pd.DataFrame()).query("data_type == 'voice'").shape[0] if "error" not in ingestion_stats else 0, delta="↑ Active")
    with col2:
        st.metric(label="🎬 Video Clips", value=ingestion_stats.get("items", pd.DataFrame()).query("data_type == 'video'").shape[0] if "error" not in ingestion_stats else 0, delta="↑ Active")
    with col3:
        st.metric(label="📝 Text Items", value=ingestion_stats.get("items", pd.DataFrame()).query("data_type == 'text'").shape[0] if "error" not in ingestion_stats else 0, delta="↑ Active")
    with col4:
        training_runs = get_training_runs(time_range)
        st.metric(label="🚀 Training Runs", value=len(training_runs), delta="Active")
    st.divider()
    ...
    if "error" not in ingestion_stats and not ingestion_stats.get("items", pd.DataFrame()).empty:
        ...
        with col1:
            fig = px.pie(df, names="data_type", title="By Data Type", color_discrete_sequence=px.colors.qualitative.Set3)
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            fig = px.pie(df, names="source_type", title="By Source", color_discrete_sequence=px.colors.qualitative.Pastel)
            st.plotly_chart(fig, use_container_width=True)
    else:
...
with tab2:
    ...
    if "error" not in ingestion_stats:
        ...
        if not ingestion_stats.get("runs", pd.DataFrame()).empty:
            ...
            st.dataframe(df_runs[["run_id", "started_at", "sources_processed", "items_ingested", "bytes_downloaded"]].head(10), use_container_width=True)
        ...
        if not ingestion_stats.get("items", pd.DataFrame()).empty:
            ...
            fig = px.line(daily, x="date", y="count", title="Items Ingested Per Day", markers=True)
            st.plotly_chart(fig, use_container_width=True)
        st.subheader("Language Distribution")
        if not ingestion_stats.get("items", pd.DataFrame()).empty:
            df = ingestion_stats["items"]
            lang_counts = df["language"].value_counts()
            fig = px.bar(x=lang_counts.index, y=lang_counts.values, title="Items by Language", labels={"x": "Language", "y": "Count"})
            st.plotly_chart(fig, use_container_width=True)
    else:
...
with tab3:
    ...
    if not training_df.empty:
        ...
        st.dataframe(training_df[["run_id", "model_type", "samples_count", "epochs", "status", "validation_score"]], use_container_width=True)
        if "validation_score" in training_df.columns:
            ...
            if not completed_runs.empty:
                fig = px.line(completed_runs, x="start_time", y="validation_score", color="model_type", title="Validation Scores Over Time", markers=True)
                st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("No training runs recorded yet.")
        st.code("python ml_training/scripts/train_unified.py --config ml_training/configs/training_config.yaml")
...
with tab4:
    ...
    if "error" not in gen_data and not gen_data.get("data", pd.DataFrame()).empty:
        ...
        with col2:
            st.metric("Avg per Generation", f"${df['cost_usd'].mean():.4f}")
        with col3:
            cache_hits = df["cache_hit"].sum() if "cache_hit" in df.columns else 0
            cache_rate = (cache_hits / len(df) * 100) if len(df) > 0 else 0
            st.metric("Cache Hit Rate", f"{cache_rate:.1f}%")
        with col4:
            st.metric("Cache Savings", f"${cache_hits * df['cost_usd'].mean():.2f}" if cache_hits > 0 else "$0.00")
        st.divider()
        if "platform" in df.columns:
            st.subheader("Cost by Platform")
            platform_costs = df.groupby("platform")["cost_usd"].sum().reset_index()
            fig = px.bar(platform_costs, x="platform", y="cost_usd", title="Total Cost by Platform", color="platform")
            st.plotly_chart(fig, use_container_width=True)
        if "model_name" in df.columns:
            st.subheader("Cost by Model")
            model_costs = df.groupby("model_name")["cost_usd"].sum().sort_values(ascending=False).head(10)
            fig = px.bar(x=model_costs.index, y=model_costs.values, title="Top 10 Models by Cost", labels={"x": "Model", "y": "Cost ($)"})
            st.plotly_chart(fig, use_container_width=True)
        st.subheader("Daily Cost Trend")
        df["date"] = pd.to_datetime(df["end_time"]).dt.date
        daily_cost = df.groupby("date")["cost_usd"].sum().reset_index()
        fig = px.area(daily_cost, x="date", y="cost_usd", title="Daily Spending", labels={"cost_usd": "Cost ($)", "date": "Date"})
        st.plotly_chart(fig, use_container_width=True)
    else:
    ...
    if "error" not in ingestion_stats and not ingestion_stats.get("items", pd.DataFrame()).empty:
        ...
        with col1:
            ...
            if "quality_score" in df.columns:
                fig = px.histogram(df, x="quality_score", nbins=20, title="Quality Score Distribution", color_discrete_sequence=["#667eea"])
                st.plotly_chart(fig, use_container_width=True)
        with col2:
            st.subheader("Quality Tier Breakdown")
            if "quality_tier" in df.columns:
                tier_counts = df["quality_tier"].value_counts()
                fig = px.pie(names=tier_counts.index, values=tier_counts.values, title="Items by Quality Tier", 
                           color_discrete_map={"excellent": "#00C853", "good": "#64DD17", "acceptable": "#FFD600", "poor": "#FF5252", "unscored": "#9E9E9E"})
                st.plotly_chart(fig, use_container_width=True)
        ...
        if "quality_score" in df.columns and "source_id" in df.columns:
            ...
            fig = px.bar(x=source_quality.index[:15], y=source_quality.values[:15], title="Top 15 Sources by Quality", labels={"x": "Source", "y": "Avg Quality Score"})
            st.plotly_chart(fig, use_container_width=True)
    else:
